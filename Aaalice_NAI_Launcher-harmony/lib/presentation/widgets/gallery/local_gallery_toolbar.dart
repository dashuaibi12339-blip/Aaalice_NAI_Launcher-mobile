...
import 'package:nai_launcher/harmony_compat.dart'; // 👈 引入咱们的终极中间件
...
class LocalGalleryToolbar extends ConsumerStatefulWidget {
  ...
  final VoidCallback? onOpenFolder; // 保留参数防止父组件报错，但UI上已弃用
...
class _LocalGalleryToolbarState extends ConsumerState<LocalGalleryToolbar> {
  ...
  Widget build(BuildContext context) {
    ...
    if (selectionState.isActive) {
      ...
      return BulkActionBar(
        ...
        onExit: () => ref.read(localGallerySelectionNotifierProvider.notifier).exit(),
        onSelectAll: () {
          if (isAllSelected) {
            ref.read(localGallerySelectionNotifierProvider.notifier).clearSelection();
          } else {
            ref.read(localGallerySelectionNotifierProvider.notifier).selectAll(allImagePaths);
          }
        },
        actions: [
          BulkActionItem(
  ...
  Widget _buildDateRangeButton(ThemeData theme, LocalGalleryState state) {
    ...
    return OutlinedButton.icon(
      ...
      style: OutlinedButton.styleFrom(
        ...
        side: hasDateRange ? BorderSide(color: theme.colorScheme.primary) : null,
      ),
    );
  }
  ...
  Future<void> _selectDateRange(BuildContext context, LocalGalleryState state) async {
    ...
    if (picked != null) {
      ref.read(localGalleryNotifierProvider.notifier).setDateRange(picked.start, picked.end);
    }
  }
  ...
  Future<void> _pickDateAndJump(BuildContext context) async {
    ...
    if (picked != null && mounted) {
      ...
      if (widget.groupedGridViewKey?.currentState != null) {
        (widget.groupedGridViewKey!.currentState as dynamic).scrollToGroup(targetGroup);
      }
