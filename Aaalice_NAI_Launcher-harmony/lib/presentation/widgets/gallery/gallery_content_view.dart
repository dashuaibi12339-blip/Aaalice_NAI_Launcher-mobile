...
class GenericGalleryContentView<T> extends ConsumerStatefulWidget {
  ...
  final void Function(LocalImageRecord record)? onSendToVibeTransfer;     // 👈 插入这行
  final void Function(LocalImageRecord record)? onSendToPreciseReference; // 👈 插入这行
  ...
  const GenericGalleryContentView({
    ...
    this.onSendToVibeTransfer,     // 👈 插入这行
    this.onSendToPreciseReference, // 👈 插入这行
    this.onDeleteItem,    
    this.emptyTitle,
    this.emptySubtitle,
    this.emptyIcon,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return GenericGalleryContentView<LocalImageRecord>(
      ...
      itemBuilder: (context, record, index, config) => LocalImageCard3D(
        ...
        // 👇 插入传给 itemBuilder 的卡片
        onSendToVibeTransfer:
            onSendToVibeTransfer != null ? () => onSendToVibeTransfer!(record) : null,
        onSendToPreciseReference:
            onSendToPreciseReference != null ? () => onSendToPreciseReference!(record) : null,
        // 👆 插入结束
      ),   
      ...
      onSendToReversePrompt: (record) async {       
        try {
          ...
          // 用 Platform.pathSeparator 从路径里切出真正的文件名
          ...
          if (!context.mounted) return;
          context.go(AppRoutes.home); // 跳转回主页
          AppToast.success(context, '图片已发送到反推模块');
        } catch (e) {
          if (context.mounted) AppToast.error(context, '发送失败: $e');
        }
      },
      onDeleteItem: (record) async {
        ...
        // 如果用户点了删除，直接从物理硬盘上把文件干掉！
        if (confirmed == true && context.mounted) {
          try {
            final file = File(record.path);
            if (await file.exists()) {
              await file.delete(); // 物理删除
            }
            // 强制通知画廊刷新
            ref.read(localGalleryNotifierProvider.notifier).refresh();
            onDeleted?.call(); 
