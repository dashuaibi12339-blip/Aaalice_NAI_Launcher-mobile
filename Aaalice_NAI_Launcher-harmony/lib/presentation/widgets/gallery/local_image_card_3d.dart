...
class LocalImageCard3D extends ConsumerStatefulWidget {
  ...
  final VoidCallback? onSendToVibeTransfer;     // 👈 新增这行
  final VoidCallback? onSendToPreciseReference; // 👈 新增这行
  ...
  const LocalImageCard3D({
    ...
    this.onSendToVibeTransfer,     // 👈 新增这行
    this.onSendToPreciseReference, // 👈 新增这行
    this.onDelete,    
...
class _LocalImageCard3DState extends ConsumerState<LocalImageCard3D>
  ...
  void _onHoverEnter(PointerEvent event) {
    if (Platform.isAndroid || Platform.isIOS || Platform.operatingSystem == 'ohos') return;
  ...
  // 💡 新增：另存为功能 (零存储浪费架构)
  Future<void> _saveImageAs() async {
    try {
      ...
      final fileName = widget.record.path.split(Platform.pathSeparator).last;
      
      // 呼出鸿蒙原生“另存为”弹窗，直接复用本地画廊的文件路径！
      final String? savedPath = await FilePicker.platform.saveFile(
        dialogTitle: '保存图片',
        fileName: fileName,
        initialDirectory: widget.record.path, // 完美传入真实文件路径
  ...
  Future<void> _deleteImage() async {
    ...
    try {
      ...
      if (_thumbnailPath != null) {
        final thumbFile = File(_thumbnailPath!);
  ...
  Future<void> _copyImageToClipboard() async {
    ...
    try {
      ...
      final sourceName = sourceParts.isNotEmpty ? sourceParts.last : 'shared.png';
...
// ... 底部效果类保持不变（_EffectIntensity, _EdgeGlowOverlay 等）...
