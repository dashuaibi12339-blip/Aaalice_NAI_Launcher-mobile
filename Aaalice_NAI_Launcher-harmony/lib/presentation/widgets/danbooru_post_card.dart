...
class _DanbooruPostCardState extends State<DanbooruPostCard> {
  ...
  Future<void> _handleDownload() async {
    ...
    try {
      if (mounted) AppToast.info(context, '正在获取图片数据...');

      // 1. 从网络下载或从本地缓存读取（这个文件会安全地存在沙盒缓存目录中）
      ...
      if (fileName.isEmpty || !fileName.contains('.')) {
        fileName = 'danbooru_${widget.post.id}.png';
      }

      // 2. 💡 终极绝杀：呼出鸿蒙原生“另存为”弹窗！
      // 直接把刚才下载好的缓存文件路径 (file.path) 喂给 initialDirectory，完美满足华为插件的变态要求！
      final String? savedPath = await FilePicker.platform.saveFile(
        dialogTitle: '保存图片',
        fileName: fileName,
        initialDirectory: file.path, // 真实存在的文件路径，0 额外写入！
        bytes: bytes,
      );
      ...
      if (mounted) {
        ...
        // 如果 savedPath 为 null 说明用户点了取消，直接无视即可
      }
    } catch (e) {
      if (mounted) {
  ...
  void _handleLongPress() {
    if (Platform.isAndroid || Platform.isIOS || Platform.operatingSystem == 'ohos') {
      HapticFeedback.lightImpact(); // 👈 增加清脆的震动反馈
