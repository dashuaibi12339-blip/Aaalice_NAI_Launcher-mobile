...
class _ExportDialogState extends ConsumerState<ExportDialog> {
  ...
  Future<void> _export() async {
    ...
    if (selectedEntries.isEmpty && selectedCategories.isEmpty) return;
    ...
    try {
      // 💡 1. 先在沙盒（临时目录）里生成 ZIP 文件，绝对有权限！
      final tempDir = await getTemporaryDirectory();
      final fileName = TagLibraryIOService().generateExportFileName();
      final tempPath = p.join(tempDir.path, fileName);
      tempFileToCleanUp = File(tempPath);
      ...
      await service.exportLibrary(
        ...
        outputPath: tempPath, // 让 service 把 ZIP 写进沙盒
      ...
      // 💡 2. ZIP 生成完毕后，呼出鸿蒙原生“另存为”弹窗
      // 把 tempPath 传给 initialDirectory，咱们的 harmony_compat 会自动把沙盒里的 ZIP 拷过去！
