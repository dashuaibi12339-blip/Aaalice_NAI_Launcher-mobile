  ...
  Future<void> importFromFiles() async {
    ...
    try {
      ...
      final result = await FilePicker.platform.pickFiles(
        ...
        allowMultiple: true,
        // 💣 核心修复 1：删掉 withData: true！绝对不能让鸿蒙插件去碰内存，会把数据搞坏！
      ...
      if (result != null && result.files.isNotEmpty) {
        ...
        for (final file in result.files) {
          ...
          // 💡 核心修复 2：优先通过沙盒绝对路径，去硬盘里读取最纯净的文件流！
          ...
          // 如果硬盘读取失败，再死马当活马医尝试读取内存 bytes
          if ((bytes == null || bytes.isEmpty) && file.bytes != null) {
            bytes = file.bytes;
          }

          // 🚨 终极拦截：如果还是空，说明文件彻底损坏，直接报错并跳过，绝不让它去请求服务器！
          if (bytes == null || bytes.isEmpty) {
            if (context.mounted) AppToast.error(context, '无法读取文件内容: $fileName');
            continue;
          }
...
// ... 保持原有逻辑不变 ...      
  ...
  // 👈 在 importFromFiles() 结束的下方，插入这段新代码
