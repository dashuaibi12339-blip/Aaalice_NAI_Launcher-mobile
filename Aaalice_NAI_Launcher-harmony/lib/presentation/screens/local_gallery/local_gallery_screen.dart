...
import '../../../core/utils/file_explorer_utils.dart'; // 👈 原作者新增
import '../../utils/krita_send_helper.dart'; // 👈 原作者新增
import '../../utils/metadata_import_applier.dart'; // 👈 原作者新增
...
class _LocalGalleryScreenState extends ConsumerState<LocalGalleryScreen> {
  ...
  Future<void> _showFirstTimeTip() async {
    // 💡 核心重构：抛弃 SharedPreferences，使用项目统一的 Hive 存储！
    final storage = ref.read(localStorageServiceProvider);
    
    // 从 Hive 中读取状态
    final hasSeenTip = storage.getHasSeenLocalGalleryTip();

    if (hasSeenTip || !mounted) return;

    // 写入状态到 Hive，鸿蒙下绝对不会丢失！
    await storage.setHasSeenLocalGalleryTip(true);
  ...
  Future<void> _reuseMetadata(LocalImageRecord record) async {
    try {
      // 【核心修复】：明确声明类型，使用单例深度读取，找回丢失的 Seed (保留手机端逻辑)
  ...
  String _resolveImportedNegativePrompt(
    ...
    // 👇 原作者修改：使用 displayNegativePrompt 替代原本的 negativePrompt
  ...
  // 👇 原作者修改：返回值从 void 改为 int，用于精准统计导入了多少个参数
