...
class _VibeExportDialogAdvancedState
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _isExporting = false;
          // 👇 使用新加的过滤方法，让报错更干净
          _errorMessage = '导出失败: ${_displayExportError(e)}';
        });
      }
    }  
  }
  ...
  Future<String?> _exportSelectedInternalVibes() async {
    ...
    // 👇 原作者新功能：如果是提取多个文件，先弹出选择文件夹的对话框
    final outputDirectory = selectedIndices.length > 1
        ? await FilePicker.platform.getDirectoryPath(
            dialogTitle: '选择 Vibe 导出文件夹',
          )
        : null;
  ...
  Future<String?> _exportEncodingFile() async {
    ...
    // 👈 【关键修复】：将文本转换为二进制数据传给移动端保存面板，绕过安卓10+的存储权限限制
    ...
    final savePath = await FilePicker.platform.saveFile(
      ...
      bytes: contentBytes, // 👈 传入二进制数据
    );

    if (savePath == null) return null;

    // 桌面端需要手动写入
    if (savePath.isNotEmpty && (Platform.isWindows || Platform.isMacOS || Platform.isLinux)) {
      await File(savePath).writeAsBytes(contentBytes);
    }

    return savePath.isEmpty ? "已保存到文件" : savePath;
  }
}
