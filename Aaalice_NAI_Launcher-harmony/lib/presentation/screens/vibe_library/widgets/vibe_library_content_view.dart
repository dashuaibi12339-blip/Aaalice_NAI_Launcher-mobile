...
class _VibeLibraryContentViewState
  ...
  Future<void> _sendEntryToGenerationWithParams(
    ...
    bool isShiftPressed, {
    required bool applyParamOverrides,     // 👈 新参数
    int? bundleChildParamOverrideIndex,    // 👈 新参数
  }) async {
    final span = VibePerformanceDiagnostics.start(
    ...
    try {
      ...
      if (!isShiftPressed && currentParams.vibeReferencesV4.length >= 16) {
        abortedReason = 'maxVibesReached';
        AppToast.warning(context, '已达到最多 16 个 Vibe 的限制');
        return;
      }
      ...
      // 🌟 核心修改 2：普通条目或 Bundle 文件不存在时，使用单个 vibe
  ...
  Future<VibeLibraryEntry?> _updateEntryParams(
    ...
    double strength,
    double infoExtracted, {
    int? bundleChildIndex, // 👈 新增参数
  }) async {
    // 🌟 新增：处理 Bundle 包中的单个子图参数保存
    if (entry.isBundle) {
      if (bundleChildIndex == null || bundleChildIndex < 0) return null;
      ...
      final bytes = await file.readAsBytes();
      final childVibes = await VibeFileParser.fromBundle(p.basename(filePath), bytes);
      if (bundleChildIndex >= childVibes.length) return null;
      ...
      if (preparedVibeData == null) {
        if (context.mounted) AppToast.error(context, '保存参数失败，Vibe 重新编码失败');
        return null;
      }

      return ref.read(vibeLibraryNotifierProvider.notifier).saveBundleChildParams(
            entry.id,
