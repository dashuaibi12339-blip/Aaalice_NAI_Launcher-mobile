...
final FakeSecureStorage _storage;
  ...
  static final Map<String, String> _memoryCache = {};
   ...
   SecureStorageService() : _storage = const FakeSecureStorage();
  // ==================== Access Token ====================
...
class FakeSecureStorage {
  ...
  // 强行把所有加密读写重定向到 Hive 的 settings 盒子中
  Future<String?> read({required String key, dynamic aOptions, dynamic iOptions, dynamic lOptions, dynamic wOptions, dynamic mOptions}) async {
    return Hive.box('settings').get('secure_$key');
  }

  Future<void> write({required String key, required String value, dynamic aOptions, dynamic iOptions, dynamic lOptions, dynamic wOptions, dynamic mOptions}) async {
    await Hive.box('settings').put('secure_$key', value);
  }

  Future<void> delete({required String key, dynamic aOptions, dynamic iOptions, dynamic lOptions, dynamic wOptions, dynamic mOptions}) async {
    await Hive.box('settings').delete('secure_$key');
  }

  Future<void> deleteAll({dynamic aOptions, dynamic iOptions, dynamic lOptions, dynamic wOptions, dynamic mOptions}) async {
    final box = Hive.box('settings');
    final keys = box.keys.where((k) => k.toString().startsWith('secure_')).toList();
